...
- **Couverture** (v8, ~55% global au 2026-08-26) : quasi 100% sur `src/lib` (détection de type, merge/split PDF via vrais documents `pdf-lib`, resize/crop via mocks Canvas/Image, pdf-to-images via mock `pdfjs-dist`, remove-background/heic-convert via mock de leurs libs externes) ; ~73% sur `src/components` (ModeSelector, ToolSelector, Dropzone, ExportButton, PreviewPanel, MergeTool, SplitTool, ResizeTool, RemoveBackgroundTool).
- **Non couvert** : `App.tsx` (orchestrateur principal, 386 lignes), `CropTool.tsx` (dépend de `react-easy-crop`), `src/workers/processing.worker.ts` (contexte Web Worker) — laissés de côté, effort de mock élevé pour un ROI plus faible que le reste.
